...
export interface HostedGitJob { repository:string; branch:string; base?:string; commitMessage:string; files:Record<string,string>; verify?:string[]; }
...
export class HostedGitExecutor {
 async execute(job:HostedGitJob):Promise<HostedGitResult>{
  ...
  try{
   ...
   const commit=(await run(repoDir,'git',['rev-parse','HEAD'])).trim();await run(repoDir,'git',['push','origin',`HEAD:refs/heads/${job.branch}`]);return {repository:job.repository,branch:job.branch,commit,verification,changed};
