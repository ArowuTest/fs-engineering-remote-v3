...
export class PersistentExecutor{
 ...
 async tick(){if(!this.running)return;try{const kinds=[...this.handlers.keys()];if(!kinds.length)return;const item=await this.queue.claim(this.workerId,kinds,this.leaseMs);if(item)await this.execute(item)}catch{}finally{this.schedule()}}
