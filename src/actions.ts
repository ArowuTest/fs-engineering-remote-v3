...
export function registerActionsRoutes(
  ...
  const requireAuth = async (request: FastifyRequest, reply: FastifyReply) => {
    if (!isAuthorized(request.headers.authorization, config.actionsSecret)) {
  ...
  app.post('/actions/fs', { preHandler: requireAuth }, async (request, reply) => respond(reply, async () => {
    ...
    if(b.action==='roots') return ops.listRoots(); if(!b.root) throw new Error('root is required.');
  ...
  app.post('/actions/process', { preHandler: requireAuth }, async (request, reply) => respond(reply, async () => {
    ...
    if(b.action==='run'||b.action==='start'){if(!b.root||!b.command)throw new Error('root and command are required.');return b.action==='run'?ops.runCommand(b.root,b.cwd??'.',b.command,b.timeoutMs):ops.startProcess(b.root,b.cwd??'.',b.command);}
  ...
  app.post('/actions/git', { preHandler: requireAuth }, async (request, reply) => respond(reply, async () => {
    ...
    switch(b.action){case'status':return ops.gitStatus(b.root,b.cwd);case'diff':return ops.gitDiff(b.root,b.cwd,b.staged??false);case'stage':return ops.gitStage(b.root,b.cwd,b.paths??[],b.all??false);case'commit':if(!b.message)throw new Error('message is required.');return ops.gitCommit(b.root,b.cwd,b.message);case'push':return ops.gitPush(b.root,b.cwd,b.remote??'origin',b.branch,b.setUpstream??false,b.forceWithLease??false);case'inspect':return ops.inspectRepository(b.root,b.cwd);}
  }));
